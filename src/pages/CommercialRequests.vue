<template>
  <q-page class="q-pa-md">
    <div class="row justify-between items-center q-mb-xl">
      <div>
        <div class="text-h4 text-weight-bold text-primary">Commercial Property Requests</div>
        <div class="text-subtitle1 text-grey-6">Browse commercial space inquiries from businesses</div>
      </div>
      <q-btn
        color="secondary"
        icon="add"
        label="Submit Business Request"
        ...
        <template #body-cell-actions="props">
          <q-td :props="props">
            <q-btn
              ...
              icon="phone"
              color="positive"
              @click="contactBusiness(props.row)"
            >
              <q-tooltip>Contact Business</q-tooltip>
            </q-btn>
            <q-btn
              flat
              round
              ...
              class="q-ml-sm"
            >
              <q-tooltip>View Requirements</q-tooltip>
            </q-btn>
          </q-td>
        </template>
      </q-table>
    ...
    <q-dialog v-model="showRequestModal" persistent>
      <q-card style="min-width: 500px">
        <q-card-section>
          <div class="text-h6 text-center">Submit Commercial Property Request</div>
        </q-card-section>
        <q-card-section class="q-pt-none">
          <q-form @submit="submitRequest" class="q-gutter-md">
            ...
            <q-select
              ...
              :options="['Office Space', 'Retail Shop', 'Warehouse', 'Restaurant Space', 'Showroom']"
              label="Space Type Needed"
              required
            />
            <q-input
              filled
            ...
            <q-input
              ...
              placeholder="Describe your space needs (sq ft, facilities, etc.)..."
              rows="3"
            />
...
export default defineComponent({
  ...
  setup() {
    ...
    const viewDetails = (request) => {
      const details = `Space Type: ${request.request}\nLocation: ${request.location}\nBudget: ${request.budget}\nRequirements: ${request.specification || 'N/A'}`;
      $q.notify({
        type: 'info',
    ...
    const submitRequest = async () => {
      try {
        await dataStore.addNewItem('commercial-requests', { ...requestForm, status: 'New', city: 'Abuja' });
        $q.notify({
          type: 'positive',
          message: 'Your commercial property request has been submitted successfully!',
          position: 'top'
        });
        closeRequestModal();
      } catch {
