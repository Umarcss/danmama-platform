  ...
  <q-page class="q-pa-md">
    ...
    <div class="row justify-between items-center q-mb-xl">
      <div>
        ...
        <div class="text-subtitle1 text-grey-6">Comprehensive land plot management system</div>
      </div>
    ...
    <div class="analytics-grid q-mb-xl">
      ...
      <div class="analytics-item">
        <q-card flat class="analytics-card available-land-card">
          <q-card-section class="q-pa-lg">
            <div class="row items-center no-wrap">
              <div class="col-auto q-mr-lg">
                <div class="stat-icon-wrapper available-land-bg">
                  <q-icon name="terrain" size="2.5rem" color="white" />
                </div>
              </div>
              <div class="col">
                <div class="stat-number">{{ availableCount }}</div>
                <div class="stat-label">Available Land</div>
                <div class="stat-trend positive">
                  <q-icon name="trending_up" size="1rem" />
                  <span>{{ landListings.length > 0 ? Math.round((availableCount / landListings.length) * 100) : 0 }}% of total</span>
                </div>
              </div>
            </div>
            <div class="progress-bar q-mt-md">
              <div class="progress-fill available-land-fill" :style="{ width: landListings.length > 0 ? `${(availableCount / landListings.length) * 100}%` : '0%' }"></div>
            </div>
          </q-card-section>
        </q-card>
      </div>
      ...
      <!-- Sold Land Card -->
      <div class="analytics-item">
        <q-card flat class="analytics-card sold-land-card">
          <q-card-section class="q-pa-lg">
            <div class="row items-center no-wrap">
              <div class="col-auto q-mr-lg">
                <div class="stat-icon-wrapper sold-land-bg">
                  <q-icon name="check_circle" size="2.5rem" color="white" />
                </div>
              </div>
              <div class="col">
                <div class="stat-number">{{ soldCount }}</div>
                <div class="stat-label">Sold Land</div>
            ...
            <div class="progress-bar q-mt-md">
              <div class="progress-fill sold-land-fill" :style="{ width: landListings.length > 0 ? `${(soldCount / landListings.length) * 100}%` : '0%' }"></div>
            </div>
          </q-card-section>
        </q-card>
      </div>

      <!-- Under Offer Land Card -->
      <div class="analytics-item">
        <q-card flat class="analytics-card under-offer-land-card">
          <q-card-section class="q-pa-lg">
            <div class="row items-center no-wrap">
              <div class="col-auto q-mr-lg">
                <div class="stat-icon-wrapper under-offer-land-bg">
            ...
            <div class="progress-bar q-mt-md">
              <div class="progress-fill under-offer-land-fill" :style="{ width: landListings.length > 0 ? `${(underOfferCount / landListings.length) * 100}%` : '0%' }"></div>
            </div>
          </q-card-section>
        </q-card>
      </div>
        ...
        <template #body-cell-status="props">
          <q-td :props="props">
            ...
            <q-select
              ...
              emit-value
              map-options
              @update:model-value="updateLandStatus(props.row, $event)"
              class="status-select"
          ...
          <q-td :props="props">
            <div class="row q-gutter-xs">
              <q-btn
                ...
                color="primary"
                size="sm"
                @click="viewLandDetails(props.row)"
              >
              ...
              >
                <q-tooltip>Delete Land</q-tooltip>
              </q-btn>
            </div>
          </q-td>
        </template>
        ...
        <div class="modal-card">
          ...
          <div class="modal-hero">
            <div class="hero-background">
              <img :src="getImageUrl(selectedLand)" alt="Land" class="hero-image">
              <div class="hero-overlay"></div>
            </div>
            <div class="hero-content">
              <div class="hero-title-section">
                <h1 class="hero-title">{{ selectedLand?.name }}</h1>
                <div class="hero-subtitle">
                  <q-icon name="location_on" size="sm" />
                  <span>{{ selectedLand?.address }}, {{ selectedLand?.city }}</span>
                </div>
              </div>
              <div class="hero-actions">
                <q-chip
                  :color="getStatusColor(selectedLand?.status)"
                  text-color="white"
            ...
            <div class="hero-price">
              <div class="price-display">{{ formatPrice(selectedLand?.price) }}</div>
              <div class="price-label">Land Price</div>
            </div>
          </div>

          <!-- Modal Content -->
          <div class="modal-content" v-if="selectedLand">
            <div class="content-grid">
              <!-- Land Information -->
              <div class="info-card glass-card">
                <div class="card-header">
                  <div class="card-icon-wrapper">
                    <q-icon name="terrain" size="md" />
                  </div>
                ...
                <div class="card-content">
                  <div class="info-row">
                    ...
                    <span class="info-value">{{ selectedLand.listingNumber || 'N/A' }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Land Type</span>
                    <span class="info-value">{{ selectedLand.type }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Location</span>
                    <span class="info-value">{{ selectedLand.city }}, {{ selectedLand.address }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Land Size</span>
                    <span class="info-value">{{ selectedLand.landSize || 'Not specified' }}</span>
                  </div>
                  <div class="info-row">
                    <span class="info-label">Landmark</span>
                    <span class="info-value">{{ selectedLand.landmark || 'N/A' }}</span>
                  </div>
                  <div class="description-section">
                    <h4 class="section-title">Description</h4>
                    <p class="description-text">{{ selectedLand.description || 'No description available' }}</p>
                  </div>
                </div>
              </div>
              ...
              <div class="info-card glass-card">
                ...
                <div class="card-content">
                  <div class="info-row">
                    ...
                    <span class="info-value">{{ selectedLand.titleDocument || 'N/A' }}</span>
                  </div>
                  <div class="info-row">
                  ...
                  <div class="description-section">
                    ...
                    <p class="description-text">{{ selectedLand.note || 'No additional notes available' }}</p>
                  </div>
                </div>
              </div>
                  ...
                  <div class="contact-info">
                    <div class="contact-item">
                      ...
                      <div>
                        ...
                        <div class="contact-value">{{ selectedLand.sellerName || 'N/A' }}</div>
                      </div>
                    </div>
                    <div class="contact-item">
                      <q-icon name="phone" />
                      <div>
                        ...
                        <div class="contact-value">{{ selectedLand.sellerContact || 'N/A' }}</div>
                      </div>
                    </div>
                    <div class="contact-item">
                      <q-icon name="email" />
                      <div>
                        ...
                        <div class="contact-value">{{ selectedLand.sellerEmail || 'N/A' }}</div>
                      </div>
                    </div>
                  </div>
                </div>
                  ...
                  <div class="contact-info">
                    <div class="contact-item">
                      ...
                      <div>
                        ...
                        <div class="contact-value">{{ selectedLand.agentName || 'N/A' }}</div>
                      </div>
                    </div>
                    <div class="contact-item">
                      <q-icon name="phone" />
                      <div>
                        <div class="contact-label">Contact Number</div>
                        <div class="contact-value">{{ selectedLand.agentNumber || 'N/A' }}</div>
                      </div>
                    </div>
                    <div class="contact-item">
                      <q-icon name="email" />
                      <div>
                        <div class="contact-label">Email Address</div>
                        <div class="contact-value">{{ selectedLand.agentEmail || 'N/A' }}</div>
                      </div>
                    </div>
                  </div>
                </div>
...
export default defineComponent({
  ...
  setup() {
    ...
    const availableCount = computed(() =>
      landListings.value.filter(l => l.status === 'Available').length
    );
    const soldCount = computed(() =>
      landListings.value.filter(l => l.status === 'Sold').length
    );
    const underOfferCount = computed(() =>
      landListings.value.filter(l => l.status === 'Under Offer').length
    );
    ...
    const getImageUrl = (land) => {
      return land.image || 'https://images.unsplash.com/photo-1441974231531-c6227db76b6e?q=80&w=2071';
    };
    ...
    const openFormForEdit = (land) => {
      itemToEdit.value = { ...land };
      showForm.value = true;
    };
    ...
    const handleDelete = async () => {
      try {
        await dataStore.deleteItem('listingsForLand', itemToDelete.value.id);
        $q.notify({
          type: 'positive',
        ...
        $q.notify({
          type: 'negative',
          message: 'Failed to delete land listing. Please try again.',
          position: 'top'
        });
      }
    };

    const updateLandStatus = async (land, newStatus) => {
      try {
        await dataStore.updateItem('listingsForLand', land.id, { ...land, status: newStatus });
        $q.notify({
          type: 'positive',
          message: `Land status updated to ${newStatus}`,
          position: 'top'
        });
        await refreshData();
      } catch {
        $q.notify({
          type: 'negative',
          message: 'Failed to update land status. Please try again.',
          position: 'top'
        });
      }
    };
...
.available-land-card::before {
  background: linear-gradient(90deg, #4CAF50 0%, #66BB6A 100%);
}

.sold-land-card::before {
  background: linear-gradient(90deg, #F44336 0%, #EF5350 100%);
}

.under-offer-land-card::before {
